...
B<Remote mode> (either 2.2.x and >=2.6.28) requires stunnel to be executed as
root.  B<setuid> option will also break this functionality.

B<Linux E<gt>=2.6.28> requires the following setup for iptables and routing
...
B<Linux 2.2.x> requires kernel to be compiled with I<transparent proxy> option.
Connected service must be installed on a separate host.
Routing towards the clients has to go through the stunnel box.

B<Local mode> works by LD_PRELOADing env.so shared library.
