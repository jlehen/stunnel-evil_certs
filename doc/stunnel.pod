...
Specifically "connect" protocol is used in client mode to establish SSL
...
=item Remote mode (B<connect> option) on B<Linux E<gt>=2.6.28>
...
B<stunnel> must also to be executed as root and without B<setuid> option.

=item Remote mode (B<connect> option) on B<Linux 2.2.x>
...
=item Remote mode (B<connect> option) on B<FreeBSD E<gt>=8.0>

This configuration requires additional firewall and routing setup.
B<stunnel> must also to be executed as root and without B<setuid> option.

=item Local mode (B<exec> option)

This configuration works by pre-loading B<libstunnel.so> shared library.
...
Original destination is used instead of B<connect> option.
...
Use both B<source> and B<destination> transparent proxy.
...
This options has been renamed to B<source>.

=item B<no>

This options has been renamed to B<none>.
...
=item B<level 1> - verify peer certificate if present

=item B<level 2> - verify peer certificate

=item B<level 3> - verify peer with locally installed certificate

=item B<default> - no verify
